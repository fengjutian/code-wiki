...
class PromptBuilder:
    ...
    def build(
        ...
        *,
        ...
        parts.append(
            ...
            "6. 注意格式：必须用换行分隔不同段落、标题、表格和列表。标题用 ## 后带空格（如 ## 标题）；表格中每行单独占一行，表头后紧跟分隔行，表格前后都要有空行\n\n"
            + role_guidance
            + "\n"
        )
    ...
    def build_synthesis(
        ...
        parts: List[str] = [
            ...
            "**要求**：\n"
            "1. 用中文撰写，专业深入但可读性强\n"
            "2. 输出纯 Markdown，不添加客套话，不要输出任何 HTML 标签。注意格式：必须用换行分隔不同段落、标题、表格和列表。标题用 ## 后带空格（如 ## 标题）；表格中每行单独占一行，表头后紧跟分隔行，表格前后都要有空行\n"
            "3. 必须包含以下章节：\n"
